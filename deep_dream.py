...
class DeepDream():
    ...
    def __init__(self, model, selected_layer, selected_filter, im_path):
        self.model = model
        self.model.eval()
        self.selected_layer = selected_layer  # 数値
        self.selected_filter = selected_filter  # 数値
        self.conv_output = 0
        # Generate a random image
        self.created_image = Image.open(im_path).convert('RGB')  # PIL Image
    ...
    def hook_layer(self):
        def hook_function(module, grad_in, grad_out):
            ...
            self.conv_output = grad_out[0, self.selected_filter]  # こちらがフィルター値がインデックスで

        # Hook the selected layer
        self.model[self.selected_layer].register_forward_hook(hook_function)  # こちらがレイヤーのインデックス

    def dream(self):
        # Process image and return variable
        self.processed_image = preprocess_image(self.created_image, True)  # type torch.Tensor
        ...
        for i in range(1, 251):
            ...
            # Backward
            loss.backward() # 最後まで計算してからlossを計算するんじゃなくて、途中でぶった切ってバックプロップしちゃう。
            # Update image
            optimizer.step()
            # Recreate image
